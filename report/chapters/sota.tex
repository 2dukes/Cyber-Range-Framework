...
Our framework addresses every column of Table \ref{tab:comparison_cr} which is not achieved by any other framework. Even in cyber range frameworks such as \textit{CyExec}, which is very complete in terms of the mentioned features, a possible idea would be to extend the development. Unfortunately, not all frameworks are open-source and some of them lack community support. Instead, we opted for creating a brand new framework featured with the functionalities we wanted. With this idea in mind, Chapter \ref{chap:problem_statement} will further detail the problem to be solved.