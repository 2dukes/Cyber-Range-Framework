...
Essentially, for every machine object passed, the contents of the \textit{teardown.yml}, as shown in Listing \ref{lst:ansible_teardown}, file are run. This uses the \textit{community.docker.docker\_container} module that is builtin in Ansible and removes the container under a given name.
...
\begin{itemize}
    ...
    \item \textit{to\_machine}: the target machine to which packets reaching the \textit{destination\_port} will be forward to.
...
After the creation of the above-mentioned SSL certificates, the newly created root CA's public-key certificate is needed in the attacker machine so it can be considered trustworthy. This way, when the trainee visits the UniFi dashboard, the website appears with a legitimate HTTPS connection. This setup is presented in Listing \ref{lst:log4j_entrypoint_script} achieved with the following entry point script:
...
\begin{figure}[H]
    \includegraphics[width=9cm]{figures/unifi_initial_dashboard.png}
...
We can also, for instance, gather information on the Domain Controller's administrators, which combine local and Domain Administrators, as shown in Fig. \ref{fig:bloodhound_dc_admins}. Here we see a randomly selected account as a local administrator.
...
\begin{figure}[H]
    \includegraphics[width=8cm]{figures/adminbot.png}
...
\begin{figure}[H]
    \includegraphics[width=10cm]{figures/ui_dashboard.png}
